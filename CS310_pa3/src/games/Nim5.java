...
public class Nim5 implements Game {
	...
	// Find optimal move
		public Best chooseMove(int side, int depth) {
			...
			int bestRow = 0; // Initialize running value with out-of-range value
			int bestSum = 0;
			int simpleEval = 0;
			int value;
	...
	public static void main(String[] args) {
		...
		try {
			...
			g.makeMove(Nim5.COMPUTER, 0, 4);// gameStrings.parseMove("A4"));
			System.out.println(g.position());
			System.out.println("doing move: B2");
			g.makeMove(Nim5.HUMAN, 1, 2);// gameStrings.parseMove("B2"));
			System.out.println(g.position());
			System.out.println("doing move: C1");
			g.makeMove(Nim5.COMPUTER, 2, 1);// gameStrings.parseMove("C1"));
			System.out.println(g.position());
			System.out.println("winner is " + g.winner());
			System.out.println("doing move: B1");
			g.makeMove(Nim5.HUMAN, 1, 1);// gameStrings.parseMove("B1"));
			System.out.println(g.position());
			System.out.println("winner is " + g.winner());
			System.out.println("doing move: A1");
			g.makeMove(Nim5.COMPUTER, 0, 1);// gameStrings.parseMove("A1"));
			System.out.println(g.position());
			System.out.println("winner is " + g.winner());
